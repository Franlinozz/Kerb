...
export function buildServer(deps: ServerDeps): FastifyInstance {
  ...
  /**
   * The open dataset (V3-09): hourly C(1%), C(3%), regime, in-range liquidity and reading counts
   * per asset since the first observation, gaps as explicit rows. Generated by
   * apps/engine/scripts/dataset-depth-hourly.ts; served as written, never recomputed here.
   */
  app.get("/v1/datasets/:file", async (req, reply) => {
    const file = (req.params as { file: string }).file;
    if (file !== "depth-hourly.json" && file !== "depth-hourly.csv") return reply.status(404).send({ error: "unknown dataset; try depth-hourly.csv or depth-hourly.json" });
    const path = resolve(repoRoot(), "data/datasets", file);
    if (!existsSync(path)) return reply.status(503).send({ error: "the dataset has not been generated yet", label: "Unavailable" });
    void reply.header("cache-control", "public, max-age=300");
    if (file.endsWith(".csv")) {
      void reply.header("content-type", "text/csv; charset=utf-8");
      void reply.header("content-disposition", "attachment; filename=\"kerb-depth-hourly.csv\"");
    } else void reply.header("content-type", "application/json; charset=utf-8");
    return reply.send(readFileSync(path, "utf8"));
  });
