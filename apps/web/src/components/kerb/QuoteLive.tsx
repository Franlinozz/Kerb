"use client";
/**
 * A live onchain read of KerbQuote.quote (V3-07, SPEC-ONCHAIN-CONSUMERS.md section 7), made
...
const ABI = parseAbi(["function quote(bytes32 assetId, uint256 collateralAmount, uint8 mode) view returns ((bool usable, uint16 regime, uint64 observedAt, uint128 creditMark, uint64 ltv, uint64 liquidationThreshold, uint256 collateralValue, uint256 maxBorrow, uint128 maxPositionDebt, uint128 debtCeiling, uint128 executableDepth1, uint64 cureDeadline, uint64 nextWeakeningAt, bytes32 inputsHash))"]);
...
export function QuoteLive({ quote, chainId, assetId, symbol }: { quote: Address; chainId: number; assetId: `0x${string}`; symbol: string }): React.ReactElement {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const q = (await c.readContract({ address: quote, abi: ABI, functionName: "quote", args: [assetId, 10n * 10n ** 18n, 0], blockNumber: block })) as Q;
        if (live) setR({ q, block });
      } catch { if (live) setR({ error: "The X Layer RPC did not answer; try the cast line below." }); }
    })();
    return () => { live = false; };
  }, [quote, chainId, assetId]);
  return (
    <div className="quote-live" aria-live="polite">
      <span className="t-label">Live read · quote({symbol}, 10 tokens, Carry) · {chainId === 196 ? "X Layer mainnet" : "X Layer testnet"}</span>
